...
```
Чтобы отделить исполнение модуля от ипортировния используется специальная переменная ____name____ (по два нижних подчеркивания). Если мы запускать модуль, то содержимое переменной ____name____ будет равно строке ____name____, а в случае импортирования – переменная ____name____ будет содержать имя модуля.
```
def sum(a,b):
    return a+b
...
True
```
Перечень популярныъ строковых методов
```
