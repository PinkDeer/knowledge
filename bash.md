...
Двойной **tab** показывает содержимое директории
**mkdir -p test/test2** - рекурсивное создание папок
**mv test1 test2** - переименовать
**mv test2/* test3** - перемещение всего содержимого
**rm file** - удаление файла
**rm -r test** - удаление папки
**rm -r test2** - удаление всего содержимого внутри директории (кроме самой директории)
**rm -r test/test2** - удаление содержимого директории из уровня выше
**rm -rf** - флаг f force, аккуратно
**cat** - вывести содержимое файла
**head** - выводит 10 первых строк
**tail** - 10 последних строк
**tail -f** - "захват" файла, удобно читать логи, выводятся 10 последних строк + новые строчки из файла
**grep 'text' file** - найти все записи из файла
**less file** - открыть файл не целиком, а только ту часть, которую видно на экране.
**q** - выход
**h** - справка
**j** - перемещение вниз по строкам
**k** - перемещение вниз по строкам
**ctrl + f** - постраничный переход вперёд
**ctrl + b** - постраничный переход назад
**shift + g** в конец файла
**g** - в начало файла
**/text** - поиск по файлу
**n** - перемещение вперёд по поиску
**shift + n** - перемещение назад по поиску
**man** - вызов мануала
**man -f mkdir** - найти упоминание во всех категориях (посмотреть категории)
1 - программная оболочка
2 - системный вызов
Например: man 2 mkdir
**tree** - просмотр файлов виде дерева
**which ls** - найти исполняемый файл программы, например ls

Переменные окружения
var=3
echo $var
env - список переменные в текущей сессии
Переменные доступны в рамках текущей сессии.
Чтобы была доступна переменная в скрипте, можно:
1) myvar=2 ./script
2) export myvar=3 - сделать переменную окружения доступную для всех
Изменить переменную PATH, чтобы добавить пусть к исполняемым файлам
PATH=/home/user/../test:$PATH
Удалить переменную
unset PATH
Перенаправления и потоки
ls > fail -записать в файл
sort < fail - сортировка из файла
sort < fail > fail - сортировка из файла и записать в новый
Конвейер: позволяет строить цепочки из программ,
Перенаправляет STDOUT одного процесса в STDIN другого
| - Для построения конвейера используется символ
История
История команд
1)cat ~/.bash_history
2) history
Выбрать по номеру: !112
!! - последняя команда
!command - последняя команда с конца начинающаяся с этого слова
ctrl-r - поиск по всей иcтории, вводить команду и нажимать ctrl+r для подбора
Псевдонимы (alias)
alias - вызов элиаса
unalias ll - удаление определённого элиаса
type ll - посмотреть не сокращенный вид
