...
Наконец, имя метода может завершаться знаком равенства (=). Методы, имена которых завершаются этим символом, используются для назначения атрибутов. В языке Ruby этот суффикс имеет специальный смысл, поэтому не применяйте его в обычных методах — или вы увидите, что
